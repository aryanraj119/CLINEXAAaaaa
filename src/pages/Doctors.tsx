...
const Doctors = () => {
  ...
  const fetchSpecializations = async () => {
    ...
    if (result.data) setSpecializations(result.data);
  };
  ...
  const fetchDoctors = async () => {
    ...
    let doctorsData: any[] = [];
    const specData = await localDb.from("specializations").select("*");
    const specMap = new Map((specData.data || []).map((s: any) => [s.id, s]));
    ...
    if (selectedSpecialization !== "all") {
      query = query.eq("specialization_id", selectedSpecialization) as any;
    }

    const { data } = await query;
    
    if (data) {
      doctorsData = (data as any[]).filter((doc: any) => doc.verified === true || doc.verified === undefined).map((doc: any) => ({
        ...doc,
