...
const DoctorDetail = () => {
  ...
  const fetchDoctor = async () => {
    ...
    const specMap = new Map((specResult.data || []).map((s: any) => [s.id, s]));
    ...
    if (data && typeof data === 'object') {
      ...
      setDoctor({
        ...doctorData,
        specialization: specMap.get(doctorData.specialization_id) || { name: '', icon: '', description: '' }
      });
      fetchReviews();
    }
    setLoading(false);
  ...
  const fetchReviews = async () => {
    const reviewsResult = await localDb
      .from("reviews")
      .select("*")
  ...
  const handleBooking = async () => {
    ...
    const { error } = await localDb.from("appointments").insert({
      patient_id: user.id,
      doctor_id: id,
